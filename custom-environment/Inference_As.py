...
chunks = [agent_keys[i:i+3] for i in range(0, len(agent_keys), 3)]
for _ in range(max_steps):
    action = {}
    for chunk in chunks:
        key_mapping = {f"agent_{i}": orig_key for i, orig_key in enumerate(chunk)}
        sub_obs = {f"agent_{i}": obs[orig_key] for i, orig_key in enumerate(chunk)}
        sub_info = {f"agent_{i}": info[orig_key] for i, orig_key in enumerate(chunk)}
        cont_actions, discrete_actions = agent.get_action(sub_obs, training=False, infos=sub_info)
        sub_action = discrete_actions if agent.discrete_actions else cont_actions
        remapped_action = {key_mapping[k]: v for k, v in sub_action.items()}
        action.update(remapped_action)
